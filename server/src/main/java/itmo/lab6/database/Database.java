...
public class Database {
     ...
     */
    public boolean insertToCollection(String login, MusicBand movie) {
        try {
  ...
  /*  public void removeByMpaaRating(String username, MpaaRating mpaaRating) {
        try {
            ...
            for (int id : movieIds) {
                MusicBand movie = getMovieById(username, id);
                if (movie != null && movie.getSinglesCount().equals(si)) {
   ...
   */
    ...
    /*  public boolean replaceLower(Integer key, MusicBand movie, String username) {
        try {
            ...
            for (int id = 0; resultSet.next(); id = resultSet.getInt(1)) {
                MusicBand movie1 = getMovieById(username, id);
     ...
     */
    private MusicBand getMovieById(String username, int id) throws SQLException {
        ...
        ResultSet result = pre.executeQuery();
        if (result.next()) {
            return (MusicBand) Serializer.deserialize(result.getBytes(1));
        }
        return null;
    }

    public boolean updateById(String username, int id, MusicBand movie) {
        try {
    ...
    public MovieCollection getCollection() {
        try {
            ...
            while (resultSet.next()) {
                ...
                MusicBand movie = (MusicBand) Serializer.deserialize(movieBytes);
